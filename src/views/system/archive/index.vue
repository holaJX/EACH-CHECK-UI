<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="archiveList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文档地址" align="center" prop="url" >
        <template slot-scope="scope">
          <span v-text="getUrl(scope.row.url)"></span>
        </template>
      </el-table-column>
...
<script>
import { listArchive, getArchive, delArchive, addArchive, updateArchive, exportArchive } from "@/api/system/archive";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        size: [
          { required: true, message: "文档大小不能为空", trigger: "blur" }
        ],
        url: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除文档管理编号为"' + archiveIds + '"的数据项?', "警告", {
